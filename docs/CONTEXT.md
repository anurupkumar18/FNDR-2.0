...
| Session continuity | `crates/fndr-memory/src/continuity.rs` | Working pure policy port: local-day/30-minute session IDs, context keys, safe URL/title anchors, candidate scoring, strict cross-app merge guard, and bounded deterministic story merge. It is model-free: callers supply similarity. Missing: a real candidate query and write-path update that uses the policy; the current scheduler deliberately still receives its session ID from its lifecycle owner. |

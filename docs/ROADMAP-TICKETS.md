...
| T-307 session identity and merge/continuity | Partial 2026-09-06 | `fndr-memory::continuity` ports the v1 30-minute session ID/key scheme, safe continuity anchors, weighted lexical/vector scoring, merge thresholds, strict 45-minute cross-app rules, and bounded deterministic story merging. `persist_capture` now selects only recent unflushed SQLite candidates, then atomically updates the original record/chunk; its FTS row follows the chunk trigger and Lance later receives one row. Tests cover session identity, safe URL parsing, same-app scoring, cross-app accept/refuse cases, small-evidence refusal, merge overlap, and the real burst write. Session-ID derivation is lifecycle-owned as of 2026-09-08: `fndr-shell::session_identity` is the application boundary that resolves local civil time (`localtime_r` behind a `LocalClock` seam) and then calls the one ported policy in `fndr-memory::continuity` - it computes no identity of its own. `StoreCaptureSink` derives session and record identity per capture from the frontmost context and the capture instant, so `RealSchedulerConfig::session_id` and the placeholder monotonic record-ID allocator are both gone; an underivable session is a typed `Persistence` pipeline failure surfaced as `persistence_failed`, never a fabricated ID (invariant 4). New record IDs are `<session_id>-<captured_at_ms>`, which no longer restarts at zero when the process does; records written by the old `desktop-<pid>-<n>` allocator keep their IDs, stay readable, and are never reinterpreted (no schema or migration change, and the two ID shapes cannot collide because a derived session always begins `YYYYMMDD-`). Shell tests cover the known-instant match against the ported policy, in-window stability plus half-hour rollover through the real sink, timezone resolution at this boundary, a restarted sink rejoining the same session, and the underivable-clock refusal. Missing: a Lance-safe indexed-record merge/update path. |

...
| T-403 model-worker priority queue | Partial 2026-09-05 | `fndr-inference/src/model_worker.rs`: `ModelWorkerHandle`/`Priority`, lazy load + idle-timeout unload, AC's contention test proves priority ordering. `scripts/check-llm-call-sites.sh` covers the "no LLM call outside the queue" AC as a standalone (not CI-wired) review-rule script. Missing: nothing in production actually calls through the queue yet — `LanceWriter::flush_once` still takes a generic `&dyn Embedder` directly. |

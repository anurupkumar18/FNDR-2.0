...
| T-306 staged capture pipeline + counters | Partial 2026-09-05 | `fndr-capture/src/pipeline.rs` provides a stage-testable synchronous pipeline with one terminal outcome/counter per tick; unit tests prove pre-pixel privacy, URL-only admission, native-signature enforcement, OCR signal delegation, dedup, and final persistence visibility. `MacOSForegroundContextSource` fails closed when metadata is unavailable. `fndr-store` migration 0003 now durably retains sanitized bundle/URL metadata for real/URL-only captures. Missing: concrete privacy/OCR/sink adapters, scheduler lifecycle, model-worker/Lance ownership, shutdown flush, and hardware permission run. |

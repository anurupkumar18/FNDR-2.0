...
| T-301 textsignal port | Done 2026-08-21, wired 2026-09-07 | PR #5 ported the crate with 15 v1 tests, but nothing consumed it for 17 days: `grep -rln "fndr-textsignal" crates/*/Cargo.toml` matched only its own manifest. Dead plumbing, the exact v1 failure mode this repo audits for, and it hid because "Done" meant "tests pass", not "on the live path". Now consumed by `fndr-shell::capture_adapters::clean_for_storage`, which runs `build_high_signal_text_for_app` on every Vision result before the quality gate and before storage. Until then every stored line carried the internal `[LOW_CONF] ` marker (Apple Vision returns 0.50 for essentially every legible line) and browser chrome was never reduced. A ticket is not Done while its only caller is its own test module. |

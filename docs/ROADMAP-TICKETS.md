...
| T-505 routes and RRF fusion | Partial 2026-09-06 | `fndr-retrieval::KeywordRetriever` is the first real route: SQLite FTS5 over durable chunks, with forward-only migration, trigger-maintained index, Porter stemming, bounded results, snippets, and stable evidence IDs. It has no model/RAM cost. Missing: Lance FTS, vector and temporal routes, metadata prefilters, RRF, timeouts/metrics, ranking, and the shared UI/MCP engine surface. |

...
| T-402 embedding contract v1 | Partial 2026-09-05 | `fndr-inference/src/embedding.rs`: `EmbeddingSpec`/`Embedder` trait seam, `CHUNK_EMBEDDING_V1` (dim 768), `query_embedding_text`/`embed_query` (instruction-prefix asymmetry) and `truncate_and_renormalize` (matryoshka rule) are done and tested (7 tests). The pinned model is downloaded to `models/` via the real `download_verified` path. Missing: the concrete Qwen3-Embedding-0.6B GGUF `Embedder` implementation itself — no llama.cpp binding crate is pinned in the workspace yet. |

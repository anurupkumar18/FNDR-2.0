...
| T-307 session identity and merge/continuity | Partial 2026-09-06 | `fndr-memory::continuity` ports the v1 30-minute session ID/key scheme, safe continuity anchors, weighted lexical/vector scoring, merge thresholds, strict 45-minute cross-app rules, and bounded deterministic story merging. Tests cover session identity, safe URL parsing, same-app scoring, cross-app accept/refuse cases, small-evidence refusal, and merge overlap. Missing: candidate retrieval and an atomic persisted merge/write path; current `StoreCaptureSink` still takes the lifecycle owner's explicit session ID. |

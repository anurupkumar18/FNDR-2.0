...
| T-702 tools wave 1 | Partial 2026-09-06 | Only 2 of the 14 named tools (ADR-007) exist: `fndr.search` and `fndr.privacy_status`. `fndr.search` now serves the same durable route the engine uses elsewhere (`fndr-retrieval::KeywordRetriever` over `fndr-store::Store`), retiring its walking-skeleton `SkeletonStore` stand-in per the AC's "served by the same engine functions" intent; it still only does plain FTS keyword search, not ADR-007's full hybrid/filtered/surfacing-reasons contract. `fndr-mcp`'s tests and the `skeleton` example were moved onto the real `Store`; `SkeletonStore` itself is untouched and still used by its own T-109 OCR round-trip test and `fndr-bench`. context_pack, timeline, recall, project_context, active_focus, source_evidence, graph_context, open_target, explain_retrieval, feedback, remember_decision are not started. |

...
| T-206 deletion everywhere | Partial 2026-09-06 | `fndr-store::delete_everywhere` resolves record, time-range, suffix-domain, or all-record scopes from SQLite; removes matching Lance rows first; then transactionally deletes SQLite truth. Tests prove a `bank.com` deletion removes root/subdomain rows but not `burbank.com`, cover record/time/all scopes, and prove record deletion empties the SQLite keyword-search result. Missing: engine/UI/MCP deletion surfaces and graph/vector-reader absence assertions. |
...
| T-301 textsignal port | Done 2026-08-21; wired 2026-09-07 | PR #5 ported the v1 module and its tests. `VisionOcrAdapter` now applies its app-aware cleanup before low-signal admission, semantic dedup, SQLite, FTS, and later Lance indexing. The live builder also corrects two donor defects: literal `[LOW_CONF]` tokens inside captured code are preserved, and its line-quality/admission math uses Unicode character counts instead of UTF-8 byte counts. |
...
| T-305 port OCR wrapper | Done (audited 2026-09-07) | `fndr-ocr/src/vision.rs`: `[LOW_CONF]` convention, `min_confidence` wired, config presets, measured short-screen regressions, and marker-free Unicode admission-floor tests. |
| T-306 staged capture pipeline + counters | Partial 2026-09-07 | `fndr-capture/src/pipeline.rs` provides a stage-testable synchronous pipeline with one terminal outcome/counter per tick; unit tests prove pre-pixel privacy, URL-only admission, native-signature enforcement, OCR signal delegation, dedup, and final persistence visibility. `MacOSForegroundContextSource` fails closed when metadata is unavailable. `fndr-shell` supplies concrete privacy/Vision/SQLite adapters plus `RealCaptureScheduler`: one queued `ModelWorkerHandle`, cadence-limited `LanceWriter` flush, and a draining shutdown flush. `VisionOcrAdapter` now emits app-aware high-signal text before semantic dedup and persistence; browser, terminal, mail, multilingual, literal-marker, and durable-FTS regressions cover the seam. App identity is typed and bundle-first as of 2026-09-08: `fndr-textsignal::AppIdentity`/`AppClass` resolve the class from `CFBundleIdentifier` and fall back to whole-token name matching, the `OcrRecognizer` contract carries the bundle ID beside the name, and `MacOSForegroundContextSource::browser_kind` matches the same way. That retires the donor's substring classifier, which read "Search" as a browser (it contains `arc`) and "Knowledge Base" as one (it contains `edge`) - in the foreground source that had asked a backgrounded browser for its front tab and attributed that URL to another app's screenshot. `capture_worker::start_real_capture_worker` owns that scheduler on a named dedicated thread, rejects a busy cadence, keeps status bounded/no-content, and joins through a final drain. `CaptureLifecycle` is the T-901 app owner: it forwards only generated, content-free `capture://status` payloads, records blocked startup reasons, and drains on `ExitRequested`. Default tests prove stored SQLite truth reaches Lance through the queue and a not-yet-due record drains on shutdown. Missing: content-free cleanup-quality aggregates in capture health, a safe reprocessing path for older raw-OCR records, a bounded hardware permission run, and the later T-902 permission UX. |
| T-307 session identity and merge/continuity | Partial 2026-09-06 | `fndr-memory::continuity` ports the v1 30-minute session ID/key scheme, safe continuity anchors, weighted lexical/vector scoring, merge thresholds, strict 45-minute cross-app rules, and bounded deterministic story merging. `persist_capture` now selects only recent unflushed SQLite candidates, then atomically updates the original record/chunk; its FTS row follows the chunk trigger and Lance later receives one row. Tests cover session identity, safe URL parsing, same-app scoring, cross-app accept/refuse cases, small-evidence refusal, merge overlap, and the real burst write. Missing: a Lance-safe indexed-record merge/update path and lifecycle-owned session-ID derivation; `StoreCaptureSink` still takes the lifecycle owner's explicit session ID. |
| T-308 adaptive sampling | Done 2026-09-06 | `fndr-capture::SamplingPolicy` implements the pure active/idle/deep-idle/forced-capture curve with a simulated timeline test. `fndr-capture::MacOSInputIdle` (a one-shot CoreGraphics query behind `InputIdleSource`) and `fndr-shell::capture_worker` are now wired together: the worker's loop replaces its fixed `recv_timeout` with policy-selected waits (`CaptureNow` ticks and re-checks via `try_recv`, `Wait(d)` blocks `recv_timeout(d)`, `DeepIdle` blocks `recv_timeout(idle_interval)`), still draining on shutdown with no polling thread and no model load. Tests cover immediate first capture, active-cadence recapture, and deep-idle pause-until-shutdown with a fake idle source. Missing: T-310's hardware soak, which was already open before this slice. |
| T-309 declarative gate policy + replay harness | Partial 2026-09-08 | The pre-pixel capture gates are now one ordered, data-driven table. `fndr-privacy::SAFETY_RULES` turns the safety gate's `if ... return` chain into rows (each an addressable `SafetyRule` with a stable `SafetyReason`), `fndr-capture::AdmissionRule` gives the browser-surface policy the same per-rule identity, and `fndr-capture::DEFAULT_GATE_RULES` composes both into eleven rows keyed by `GateId`, each with a stage, an action, and an `enabled` flag. `PolicyGate` is the one seam the pipeline consults: `run_tick` lost its inline admission branch, `GateDecision` gained `UrlOnly`, and the shell's hand-written `SafetyDecision`-to-`SkipReason` match is gone (`PrivacyGate` is now an alias). `fndr-capture::replay` scores a checked-in synthetic corpus (18 fixtures, `fixtures/capture-gate-replay.json`, one per gate class plus four that must survive) through that same policy object and reports per-gate dropped/url-only/redacted counts, listing gates that fired zero times; `ReplayReport::delta` gives the per-gate drop deltas the AC asks for. `cargo run -p fndr-capture --example gate_replay` prints the report offline with no permissions or hardware. The named regression `stacked_gates_regression_shows_a_per_gate_delta_not_a_silent_zero` reproduces the v1 stacked-gates failure (one careless one-character auth indicator suppresses every stored fixture) and asserts the report names the widened gate as the largest movement and shows downstream gates starving to zero; it was verified to fail when the widening is removed. No gate was widened by the restructure: `the_table_reproduces_the_pre_table_decisions_exactly` differentially checks the table against `fndr_privacy::evaluate` plus `classify_capture_surface_policy`, and the rule order of both tables is asserted whole. `SkipReason` is unchanged: no counter appears or disappears. Missing: per-`GateId` counters at runtime (`CaptureCounters` still aggregates into `PreCapturePrivacy`/`AdmissionPolicy`, so live attribution is coarser than replay attribution; T-1007 capture-explain needs this), post-OCR gates in the replay path other than the secret scan (low-signal and semantic dedup are pipeline stages, not policy rows), owner-editable policy loaded from settings, and a CI job that fails a PR on an unexplained replay delta. |
| T-310 ScreenCaptureKit soak | Partial 2026-09-06 | The soak finally has an instrument: `cargo run -p fndr-shell --example capture_soak` owns `start_real_capture_worker` for a bounded number of minutes, prints per-outcome tick counts and the shutdown drain, samples its own RSS every 15s and reports start/end/peak/growth for the AC's leak trend, and exits non-zero when zero ticks occurred (a silent soak is a finding, not a pass). This is the first thing in the repo to own the real worker, and it is a CLI, not the desktop lifecycle T-901 owes. Missing everything that needs a human at the keyboard: the multi-day run itself, the RSS trend call, the periodic-capture-vs-persistent-SCStream decision, the fallback note, and the macOS 26.1 bundle/TCC quirk write-up. The runner captures the operator's screen, so it is run deliberately by a person and never unattended by an agent. |
...
| T-505 routes and RRF fusion | Partial 2026-09-06 | `fndr-retrieval::KeywordRetriever` is the first real route: SQLite FTS5 over durable chunks, with forward-only migration, trigger-maintained index, Porter stemming, bounded results, snippets, and stable evidence IDs. `VectorRetriever` now reads the same rebuildable Lance chunk derivative with the contract's query-side embedding and returns only stable IDs, source/time metadata, and raw distance; it does not fuse scores. `fndr-bench --route vector` requires an explicit GGUF path and an empty disposable work directory, while `make bench` remains the FTS-only CI gate. The local Qwen/M1 synthetic smoke run measured Recall@5/MRR@10 1.0000 and p50/p95 90.43/90.62 ms; it is not a committed quality baseline. `fndr.search` (fndr-mcp) now calls `VectorRetriever` directly and merges its results with the keyword route when the MCP server is launched with `--model`/`--index-dir`, proven by a real end-to-end test against the pinned Qwen3-Embedding-0.6B model (`crates/fndr-mcp/tests/vector_route.rs`). Missing: Lance FTS, temporal routes, metadata prefilters, RRF, timeouts/metrics, ranking, and the shared UI engine surface. |
...
| T-702 tools wave 1 | Partial 2026-09-06 | 12 of the 14 named tools (ADR-007) exist, past the ADR's "month 2, first 8 tools" action item: `fndr.search`, `fndr.context_pack`, `fndr.privacy_status`, `fndr.timeline`, `fndr.delta`, `fndr.active_focus`, `fndr.source_evidence`, `fndr.open_target`, `fndr.recall`, `fndr.explain_retrieval`, `fndr.feedback`, `fndr.remember_decision`. `fndr.search` now serves the same durable route the engine uses elsewhere (`fndr-retrieval::KeywordRetriever` over `fndr-store::Store`), retiring its walking-skeleton `SkeletonStore` stand-in per the AC's "served by the same engine functions" intent; it now also serves the vector route (merged with keyword hits, not fused or ranked together) when the MCP server is launched with `--model`/`--index-dir`, but still not ADR-007's full hybrid/filtered/surfacing-reasons contract. `fndr.timeline` groups activity counts per app per hour/day bucket through `Store::activity_buckets`, corrects bucket boundaries for a caller-supplied UTC offset, flags a `limit`-clipped result as `truncated`, and never carries capture text. `fndr.source_evidence` resolves a search hit's `record_id` to its capture metadata and chunk shape through `Store::record_evidence`, with the stored capture text behind an `include_raw` gate that defaults closed and a `raw_included` echo so the gate state is never inferred; an unknown id is a typed refusal. `fndr.remember_decision` is the only write tool: it appends to schema-v1's `decision_ledger` via `Store::remember_decision`, rejects an empty statement before any write, and never edits or removes prior entries. `fndr.context_pack` is the headline: it runs the keyword route and packs capture text until an estimated token budget is spent, citing record/chunk/app/time on every item, reporting `retrieval_route: keyword` so no caller assumes semantic recall, `estimated_tokens_used` (four chars per token; there is no tokenizer on this path) and `dropped_for_budget` so a thin pack is not read as a thin memory. It has no `include_raw` gate because carrying text is its purpose, so it is audited as a raw release on every call. `fndr.explain_retrieval` reports the terms the FTS index actually sees, the `all_terms` AND semantics that cause most "found nothing" cases, total matches versus what a limit would drop, and two notes on its own blind spots (privacy exclusion happens at capture so it cannot show as dropped here; only the keyword route exists). `fndr.feedback` records a rating in migration 0006's `result_feedback` and returns `ranking_changed: false` explicitly; nothing reads that table into a ranker, it stores the query (unlike the audit log) because feedback without its query cannot be replayed as an eval case, and its FK is `SET NULL` so deleting a rated memory cannot rewrite eval history. `fndr.active_focus` reports the newest capture with its age and a typed `active`/`stale`/`none` status, so a stale observation cannot be read as what someone is currently doing (its project/task inference is unbuilt: no data model). `fndr.delta` summarizes captures since an instant through `Store::changes_since` (totals plus busiest apps, counts only) and returns a `newest_captured_at_ms` cursor for cheap repeated polling; its total counts every app even when the app list is capped. `fndr.open_target` resolves a record to its sanitized URL or its app bundle, and returns an explicit `unavailable` kind with a reason when it kept neither, reusing `Store::record_evidence` rather than widening `CaptureMetadata` (whose two-field shape is itself a privacy assertion under test). `fndr.recall` reads that ledger back through `Store::recent_decisions` (newest first, `since_ms`/`limit` bounded), closing the write-only loop; its `error`/`blocker`/`todo` kinds are typed refusals rather than empty lists, because an empty list reads as "nothing was recorded". `docs/mcp.md` now carries the per-tool contract ADR-007's tool-addition rule requires. `fndr-mcp`'s tests and the `skeleton` example were moved onto the real `Store`; `SkeletonStore` itself is untouched and still used by its own T-109 OCR round-trip test and `fndr-bench`. Every tool call is recorded in migration 0005's `mcp_audit` table (tool, outcome, and whether raw capture text was released, and nothing else); auditing is structural, since each `#[tool]` method is a thin wrapper over a private `_inner` and a test asserts the audited set equals `registered_tool_names()`, so a new tool cannot ship unaudited. Missing: audit retention/UI, per-tool rate limits (only one global window exists), ADR-007's flexible `time_window` shorthand (tools take explicit unix-ms bounds), ADR-007's `depth` parameter and the pack's citation-quality/relevance gates, and project_context and graph_context, both blocked on absent data models (no project entity; nothing writes the graph tables) rather than on effort. |
...
- **T-804 · Pause, incognito, menu-bar controls** `lane::frontend` `prio::p0` `M1` deps: T-306
  Partial 2026-09-06: the status window and menu bar now issue an explicit pause/resume command to the existing worker. The worker acknowledges before `capture://status` publishes `paused`, then blocks new capture opportunities until resume while preserving the final shutdown drain; a regression test proves no tick occurs during the pause. Existing private/incognito title cues now map to a distinct, content-free `private_browsing` pre-pixel skip state instead of a generic privacy skip; this is not browser-native incognito coordination. Missing: robust browser-native incognito/review coordination, persistable privacy controls, and the human GUI rehearsal. AC: toggle reflects in UI within one tick; incognito also pauses review work.
...
- **T-901 · Tauri shell foundation** `lane::platform` `prio::p0` `M1` deps: T-101
  Partial 2026-09-06: `fndr-shell` is now a headless-first Tauri host with an app-owned capture lifecycle, generated status command/event payload, explicit `--data-dir`/`--model` demo overrides, and a bounded `--run-seconds` clean-exit path. Normal launch opens a non-capturing trust/status window; only its explicit **Start capture** action can begin the worker and request Screen Recording. Closing an active window hides it rather than ending capture. Menu-bar **Show FNDR** and **Quit FNDR** controls restore the window or perform the blocking exit drain. Its explicit pause/resume controls wait for a worker acknowledgement before changing the content-free lifecycle state and never abandon an in-flight write. A non-blocking advisory lock retained in the selected data directory prevents a second host from opening the same SQLite/Lance writer; the supported single-instance callback instead foregrounds the first main window on a second graphical launch. Login autostart remains open. `fndr-mcp` now also has a durable-store launcher for the same local SQLite file. AC evidence pending: run the actual shell with a permitted screen and real embedding model, then observe capture plus clean exit drain.
- **T-902 · Permissions onboarding** `lane::platform` `prio::p0` `M2` deps: T-901, T-302
  Partial 2026-09-06: normal desktop launch now renders a local trust/status window before capture construction. It shows Core Graphics' current non-prompting Screen Recording preflight; an explicit **Start capture** action remains the only normal path that can request Screen Recording. A ScreenCaptureKit permission/tool failure has a content-free `screen_recording_or_capture_unavailable` tick reason and directs the owner to System Settings before the next capture opportunity; it does not claim to know whether macOS denied or revoked a particular request. The same window provides a one-click, bounded, read-only view of the local `mcp_audit` ledger (time, tool, outcome, raw-release flag only); an absent vault stays absent and reports no recorded activity. Missing: live egress counter, incognito coordination, real permission/revocation probe, audit retention, and the human GUI rehearsal. AC: denial and revocation states have guidance, never silence; the trust screen renders before the prompt.
...
- **T-907 · fndr doctor and setup diagnostics** `lane::platform` `prio::p0` `M2` deps: T-901
  Partial 2026-09-06: `fndr-shell --doctor` is a read-only preflight for the explicit demo `--data-dir`/`--model` paths. It reports existing/will-create/invalid data-path state, model-ready/missing state, and Core Graphics' current `screen_recording: granted` or `not_granted` preflight result; it never calls the separate permission-request API, launches Tauri, opens ScreenCaptureKit, OCR, or creates a store. `ready_for_permission_rehearsal` remains a path/model readiness result, not a permission claim. Missing: the status panel, real permission/revocation probe, store/index/sidecar/MCP checks, exportable report, and the seeded-fault matrix AC.

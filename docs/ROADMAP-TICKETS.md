...
| T-307 session identity and merge/continuity | Partial 2026-09-08 | `fndr-memory::continuity` ports the v1 30-minute session ID/key scheme, safe continuity anchors, weighted lexical/vector scoring, merge thresholds, strict 45-minute cross-app rules, and bounded deterministic story merging. `persist_capture` atomically updates the original record/chunk; its FTS row follows the chunk trigger. The Lance-safe indexed-record merge/update path landed 2026-09-08 (`fndr-store/src/indexed_merge.rs`, migration 0007, ADR-002 amendment): `chunks.index_state` (`fndr_types::ChunkIndexState` Pending/Indexed/Superseded) plus a `revision` token let an already-flushed record be merged in SQLite alone, after which the flush cycle deletes the stale Lance row by chunk id, marks the batch, adds, and stamps only the embedded revision. Ordering is truth-first (the opposite of `delete_everywhere`, for the opposite safety property) with a written crash-window analysis in the module doc comment; `PersistCaptureOutcome::Merged` now reports `index_repair_pending` and `FlushReport` reports `stale_rows_removed` and `raced_by_merge`. Tests cover session identity, safe URL parsing, same-app scoring, cross-app accept/refuse cases, small-evidence refusal, merge overlap, the real burst write, an indexed-candidate merge through `persist_capture`, the lost-stamp and failed-embed crash windows, a real two-connection merge racing a flush, a refused stale-candidate merge, and an unknown on-disk discriminant. Side effect: the previously accepted duplicate-on-crash window in `flush_once` is closed. Missing: lifecycle-owned session-ID derivation; `StoreCaptureSink` still takes the lifecycle owner's explicit session ID. |

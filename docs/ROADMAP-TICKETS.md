...
| T-301 textsignal port | Done 2026-08-21; wired 2026-09-07 | PR #5 ported the v1 module and its tests. `VisionOcrAdapter` now applies its app-aware cleanup before low-signal admission, semantic dedup, SQLite, FTS, and later Lance indexing. The live builder also corrects three donor defects: literal `[LOW_CONF]` tokens inside captured code are preserved, its line-quality/admission math uses Unicode character counts instead of UTF-8 byte counts, and the adjacent snippet and chrome-noise paths (`is_useful_snippet_line`, `reduce_chrome_noise_for_app`, `estimate_noise_score`, and the tab-strip/pipe-row/compact-caption shape tests) now count characters too, so a CJK or Cyrillic line is not charged 2-3x for the same information. `looks_like_feed_fragment` deliberately keeps its byte length, documented in place: it only bounds a whitespace word count that is meaningless without inter-word spaces. |

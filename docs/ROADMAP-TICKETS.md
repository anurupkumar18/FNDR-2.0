...
| T-402 embedding contract v1 | Partial (audited 2026-09-05) | `fndr-inference/src/embedding.rs` has the `EmbeddingSpec`/`Embedder` trait seam and `CHUNK_EMBEDDING_V1` (dim 768) only; its own doc comment says the real implementation is not yet written. Missing: the actual Qwen3-Embedding-0.6B GGUF integration, instruction-asymmetry and truncate+L2-renormalize logic, and their tests. The writer-side wrong-dimension refusal is already tested (`fndr-store` `lance_flush.rs`), but that is downstream of this contract, not part of it. |
